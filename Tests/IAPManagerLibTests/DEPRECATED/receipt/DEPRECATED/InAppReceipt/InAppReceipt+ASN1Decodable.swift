...
class _InAppReceipt {
   ...
   init(rawData: Data) throws {
      let asn1decoder: ASN1Decoder = .init()
...
extension _InAppReceipt {
   var digestAlgorithm: SecKeyAlgorithm? {
      guard let algName: String = pkcs7container.signedData.alg.items.first?.algorithm else {
         return nil
      }

      guard let alg: SecKeyAlgorithm = OID(rawValue: algName)?.encryptionAlgorithm() else {
         return nil
      }
   ...
   var worldwideDeveloperCertificateData: Data? {
      let arr: [PKCS7Container.Certificate] = pkcs7container.signedData.certificates.certificates
   ...
   var signatureData: Data {
      pkcs7container.signedData.signerInfos.encryptedDigest
   }

   var iTunesCertificateContainer: PKCS7Container.Certificate? {
      pkcs7container.signedData.certificates.certificates.first
   }

   var iTunesCertificateData: Data? {
      iTunesCertificateContainer?.rawData
   }

   var iTunesPublicKeyData: Data? {
      iTunesCertificateContainer?.cert.subjectPublicKeyInfo
   }
}

extension InAppReceiptPayload: ASN1Decodable {
   static var template: ASN1Template {
      ASN1Template.universal(ASN1Identifier.Tag.octetString)
   }
   ...
   enum CodingKeys: ASN1CodingKey {
      ...
      var template: ASN1Template {
         ASN1Template.universal(ASN1Identifier.Tag.set).constructed()
      }
   }

   init(from decoder: Decoder) throws {
      let asn1d: ASN1DecoderProtocol = decoder as! ASN1DecoderProtocol
      ...
      var container: ASN1UnkeyedDecodingContainerProtocol = try c.nestedUnkeyedContainer(forKey: .set) as! ASN1UnkeyedDecodingContainerProtocol
      while !container.isAtEnd {
         do {
            var attributeContainer: ASN1UnkeyedDecodingContainerProtocol = try container.nestedUnkeyedContainer(for: ReceiptAttribute.template) as! ASN1UnkeyedDecodingContainerProtocol
            let type: Int32 = try attributeContainer.decode(Int32.self)
            _ = try attributeContainer.skip(template: .universal(ASN1Identifier.Tag.integer)) // Consume
            var valueContainer: ASN1UnkeyedDecodingContainerProtocol = try attributeContainer.nestedUnkeyedContainer(for: .universal(ASN1Identifier.Tag.octetString)) as! ASN1UnkeyedDecodingContainerProtocol
            switch type {
...
extension InAppPurchase: ASN1Decodable {
   public init(from decoder: Decoder) throws {
      var container: ASN1UnkeyedDecodingContainerProtocol = try decoder.unkeyedContainer() as! ASN1UnkeyedDecodingContainerProtocol
...
struct ReceiptAttribute: ASN1Decodable {
   static var template: ASN1Template {
      ASN1Template.universal(ASN1Identifier.Tag.sequence).constructed()
   }
...
struct PayloadContainer: ASN1Decodable {
   ...
   static var template: ASN1Template {
      ASN1Template.contextSpecific(0).constructed().explicit(tag: ASN1Identifier.Tag.octetString).constructed()
   }
...
struct _PayloadContainer: ASN1Decodable {
   ...
   static var template: ASN1Template {
      ASN1Template.contextSpecific(0).constructed()
   }
