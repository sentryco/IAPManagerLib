...
extension ProductType: RawRepresentable {
   ...
   public init(from decoder: Decoder) throws {
      ...
      if let monthlySub: IAPMonthlySubProduct = try? container.decode(IAPMonthlySubProduct.self, forKey: .monthlySub) { self = .monthlySub(monthlySub); return }
      if let nonConsumable: IAPNonConsumable = try? container.decode(IAPNonConsumable.self, forKey: .nonConsumable) { self = .nonConsumable(nonConsumable); return }
